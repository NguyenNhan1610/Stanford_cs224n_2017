...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    ### YOUR CODE HERE (5-20 lines)
        ...
        w = [new_sentence[i:i+(window_size*2)+1] for i in xrange(len(sentence))]
        w = [[subsubitem for subitem in item for subsubitem in subitem] for item in w]
        windowed_data.extend( [(w[i], labels[i]) for i in xrange(len(sentence))] )
...
class WindowModel(NERModel):
    ...
    def add_placeholders(self):
        ...
        self.labels_placeholder =  tf.placeholder(tf.int32, [None])    #not one hot vectors, integers
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable("W", [self.config.n_window_features * self.config.embed_size, self.config.hidden_size], initializer=xavier_initializer)
        
        b1 = tf.get_variable("b1", [1,self.config.hidden_size], initializer=zeros_initializer)
        
        U = tf.get_variable("U", [self.config.hidden_size, self.config.n_classes], initializer=xavier_initializer)
        
        b2 = tf.get_variable("b2", [1,self.config.n_classes], initializer=zeros_initializer)
    ...
    def add_loss_op(self, pred):
        ...
        #loss = tf.reduce_mean( tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred,labels=tf.to_float(self.labels_placeholder)) )
