...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        new_sentence = [sentence[i] if i < sen_len else zero_vector for i in xrange(max_length)]
        new_lables =  [labels[i] if i < sen_len else zero_label for i in xrange(max_length)]
        mask =  [True if i < sen_len else False for i in xrange(max_length)]
        ret.append((new_sentence, new_lables, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        U = tf.get_variable("U", [self.config.hidden_size, self.config.n_classes], initializer= zeros_initializer)
        b2 = tf.get_variable("b2", [1,self.config.n_classes], initializer=zeros_initializer)
        h = tf.zeros([tf.shape(x)[0],tf.shape(x)[2]])
        ### END YOUR CODE
...
#                print x[:,time_step].get_shape().as_list()
